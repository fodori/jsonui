...
const RadioGroupField = (props: Record<string, unknown>) => {
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    props?.onChange({ target: { value: parsed } })
  }
  ...
  return (
    <>
      ...
      {(options as unknown[]).map((i: unknown) => {
        ...
        return (
          <div key={itemValue}>
            <input {...ownProps} value={JSON.stringify(i.value)} onChange={handleChange} type="radio" checked={checked} id={`radio-${itemValue}`} />
            <label htmlFor={`radio-${itemValue}`}>{i.label}</label>
          </div>
        )
      })}
