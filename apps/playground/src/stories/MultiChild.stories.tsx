...
import { JsonUI, actions, builtinComponents, modifiers, useControlledInputValue } from '@jsonui/react'
...
const EditMultiChild = (props: Record<string, unknown>) => {
  ...
  if (error && fieldErrors) {
    helperText = Array.isArray(fieldErrors) ? fieldErrors.join(', ') : fieldErrors
  }
  if (validation && inputValue && validation.jsonataDef) {
    let isValid = true
    try {
      ...
      if (result && typeof result === 'object' && 'then' in result && typeof (result as { then: unknown }).then === 'function') {
        // Async results are not expected in this story; treat as invalid.
        isValid = false
      } else {
