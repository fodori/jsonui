/* eslint-disable */
import { calculatePropsFromModifier, getParentProps, getCurrentPaths, getFilteredPath, processModifiers } from './wrapperUtil'
...
test('test getParentProps', () => {
  ...
  expect(getParentProps({ a: 'aa', $childMain: 'test', $childTop: 'test', $child: 'test', $children: 'test' } as unknown as PropsType)).toStrictEqual({
    a: 'aa',
  })
  expect(getParentProps({ a: 'aa', ab: 'aa', $childMain: 'test' } as unknown as PropsType)).toStrictEqual({ a: 'aa', ab: 'aa' })
})

test('test getFilteredPath', async () => {
  const aaa = {
    $comp: 'Edit',
    value: { $modifier: 'get', store: 'data', path: 'age1' },
    error: { $modifier: 'get', store: 'data', path: { $modifier: 'get', store: 'data', path: { $modifier: 'get', store: 'data', path: 'age11' } } },
    label: 'Validation',
    helperText: "it's a child age",
    onChange: { $modifier: 'get', store: 'data5', path: { $modifier: 'get', store: 'data', path: 'age21' } },
  }
  // const aaa = { $modifier: 'get', store: 22, path: 'ssss' }
  const stock = new Stock(
    {
      components: {},
      functions: {
        get: (attr: any, props: any, callerArgs: any, stock: any) => {
          // Your get logic here - e.g., retrieve from store
          // console.log('Getting with attr:', attr)
          // console.log('Getting with props:', props)
          // console.log('Getting with callerArgs:', callerArgs)
          return `retrieved_${attr.store}_${attr.path}`
        },
      },
    },
  ...
  expect(await processModifiers(aaa, stock, reduxPaths)).toStrictEqual({
    $comp: 'Edit',
    error: 'retrieved_data_retrieved_data_retrieved_data_age11',
    helperText: "it's a child age",
    label: 'Validation',
    onChange: 'retrieved_data5_retrieved_data_age21',
    value: 'retrieved_data_age1',
  })
})
