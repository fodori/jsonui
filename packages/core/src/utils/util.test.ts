...
test('hasLeaf test emptyStringAllowed false', () => {
  expect(util.hasLeaf('', false)).toBe(true)
  expect(util.hasLeaf('a', false)).toBe(false)
  expect(util.hasLeaf(6, false)).toBe(false)
  expect(util.hasLeaf(BigInt(9007199254740991), false)).toBe(false)
  expect(util.hasLeaf(null, false)).toBe(true)
  expect(util.hasLeaf(undefined, false)).toBe(true)
  expect(util.hasLeaf({}, false)).toBe(true)
  expect(util.hasLeaf([], false)).toBe(true)
  expect(util.hasLeaf(false, false)).toBe(false)
  expect(util.hasLeaf(true, false)).toBe(false)
  const a: any = { a: 2, b: 5 }
  expect(util.hasLeaf(a, false)).toBe(false)
  a.b = a
  expect(util.hasLeaf(a, false)).toBe(false)

  expect(util.hasLeaf({ a: { c: [] }, b: [], d: [], e: { g: {} }, w: null, t: undefined }, false)).toBe(true)
  expect(util.hasLeaf({ a: { c: [5] }, b: [], d: [], e: { g: {} }, w: null, t: undefined }, false)).toBe(false)
  expect(util.hasLeaf([], false)).toBe(true)
  expect(util.hasLeaf([[[[{}], {}], null, undefined], { w: null, t: undefined }], false)).toBe(true)
})
