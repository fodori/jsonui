...
test('hasLeaf test emptyStringAllowed false', () => {
  expect(util.hasLeaf('', false)).toBe(false)
  expect(util.hasLeaf('a', false)).toBe(true)
  expect(util.hasLeaf(6, false)).toBe(true)
  expect(util.hasLeaf(BigInt(9007199254740991), false)).toBe(true)
  expect(util.hasLeaf(null, false)).toBe(false)
  expect(util.hasLeaf(undefined, false)).toBe(false)
  expect(util.hasLeaf({}, false)).toBe(false)
  expect(util.hasLeaf([], false)).toBe(false)
  expect(util.hasLeaf(false, false)).toBe(true)
  expect(util.hasLeaf(true, false)).toBe(true)
  const a: any = { a: 2, b: 5 }
  expect(util.hasLeaf(a, false)).toBe(true)
  a.b = a
  expect(util.hasLeaf(a, false)).toBe(true)

  expect(util.hasLeaf({ a: { c: [] }, b: [], d: [], e: { g: {} }, w: null, t: undefined }, false)).toBe(false)
  expect(util.hasLeaf({ a: { c: [5] }, b: [], d: [], e: { g: {} }, w: null, t: undefined }, false)).toBe(true)
  expect(util.hasLeaf([[[[{}], {}], null, undefined], { w: null, t: undefined }], false)).toBe(false)
})
